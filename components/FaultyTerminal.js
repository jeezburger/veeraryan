...
// Shader code (YOURS, NOT MODIFIED)
const vertexShader = `
...
export default function FaultyTerminal({
    ...
    mouseReact = true,
    mouseStrength = 0.2,
    dpr: propDpr, // user prop override for dpr
    ...
    // SSR-safe state for DPR
    const [dpr, setDpr] = useState(1);

    useEffect(() => {
        // Only update dpr on client
        if (typeof window !== "undefined") {
            setDpr(propDpr ?? Math.min(window.devicePixelRatio || 1, 2));
        }
    }, [propDpr]);

    // Memoize derived values for prop changes
    ...
    useEffect(() => {
        // Only run on client AND when dpr is ready
        const ctn = containerRef.current;
        if (!ctn || !dpr) return;

        let destroyed = false;
        let renderer, gl, program, mesh, resizeObserver;
    ...
    // --- React DOM rendering (component root) ---
    return (
        <div
            ref={containerRef}
            className={className}
